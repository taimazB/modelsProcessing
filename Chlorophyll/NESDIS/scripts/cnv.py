...
ncFile = Dataset("NESDIS_Chlorophyll_%s.nc" % date, 'r')
chlr = np.flipud(ncFile.variables['chlor_a'][0,0].data)
missingValue = -999
chlr[chlr == missingValue] = np.nan
latNC = np.flip(ncFile.variables['latitude'][:].data)
lonNC = ncFile.variables['longitude'][:].data
...
def saveImg(i, j):
    devNull = os.system('mkdir -p ../tiles/NESDIS_Chlorophyll_%s_12/%d/%d' % (date, zoom, i))
    ...
    if(np.isnan(np.nanmax(chlrLog))):
        ...
        # Saving
        imageio.imwrite('../tiles/NESDIS_Chlorophyll_%s_12/%d/%d/%d.png' % (date, zoom, i, 2**zoom-j-1), np.flipud(chlrColored).astype(np.uint8))
