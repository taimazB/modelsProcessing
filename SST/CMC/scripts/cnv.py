...
# tracemalloc.start()
dateTime = sys.argv[1].split('_')
date = dateTime[0]
hr = dateTime[1]

ncFile = Dataset("CMC_Currents_%s_%s.nc" % (date, hr), 'r')
sst = ncFile.variables['sst'][0].data
missingValue = -9e+33
...
def saveImg(i, j):
    ...
    if((i+1)*tileSize > iLonMax):
        if(i*tileSize > iLonMax):
            ...
            sstNew[:,iLonMax % tileSize:] = np.nan
    if(np.isnan(np.nanmax(sstNew))):
        ...
        # Saving
        imageio.imwrite('../tiles/CMC_SST_%s_%s/%d/%d/%d.png' % (date, hr,
