...
var = ncFile.variables['swh'][0].data
...
colors1 = colorRange([0, 51, 153],[0, 255, 204],200)
colors2 = colorRange([0, 255, 204],[51, 204, 51],200)
colors3 = colorRange([51, 204, 51],[204, 204, 0],200)
colors4 = colorRange([204, 204, 0],[204, 0, 0],200)
colors5 = colorRange([204, 0, 0],[255, 204, 204],200)
...
for iSub in np.arange(noOfPieces):
    ...
    for zoom in np.arange(2,8):
        ...
        if zoom<=5:
            ...
            varNewInt[varNewInt<varMin] = varMin
            ...
            for i in np.arange(int(2**zoom/noOfPieces)):
                for j in np.arange(2**zoom):
                    devNull = os.system('mkdir -p ../tiles/CMC_SWH_%s_%02d/%d/%d' % (datetime.strftime(dateSave, "%Y%m%d"),hrSave,zoom,i+iSub*int(2**zoom/noOfPieces)))
                    ...
                    with open('../tiles/CMC_SWH_%s_%02d/%d/%d/%d.png' % (datetime.strftime(dateSave, "%Y%m%d"),hrSave,zoom,i+iSub*int(2**zoom/noOfPieces),2**zoom-j-1), 'wb') as f:
            ...
            for iZoomSub in np.arange(xChunks):
                ...
                for i in np.arange(8):
                    for j in np.arange(2**zoom):
                        devNull = os.system('mkdir -p ../tiles/CMC_SWH_%s_%02d/%d/%d' % (datetime.strftime(dateSave, "%Y%m%d"),hrSave,zoom, i+8*iZoomSub+8*xChunks*iSub))
                        varSub = varColored[j*tileSize:(j+1)*tileSize,i*tileSize:(i+1)*tileSize,:]
                        img2list = np.flipud(varSub).reshape(-1, 3*varSub.shape[1]).astype(np.uint8)
                        with open('../tiles/CMC_SWH_%s_%02d/%d/%d/%d.png' % (datetime.strftime(dateSave, "%Y%m%d"),hrSave,zoom, i+8*iZoomSub+8*xChunks*iSub, 2**zoom-j-1), 'wb') as f:
                            writer = png.Writer(width=varSub.shape[1], height=varSub.shape[0], bitdepth=8, greyscale=False)
                            writer.write(f, img2list)
