...
ls /media/taimaz/mapTiles/SWH/CMC/tiles | cut -d_ -f3,4 > /media/taimaz/mapTiles/SWH/CMC/dateTimes
EOF
    log "`date` - SWH CMC - rsynched - DONE"
}
...
if [[ ! -z ${lastAvailDate} ]] && [[ ${lastAvailDate} != ${lastDlDate} ]]; then
    ...
    log "`date` - SWH CMC - Downloade & grib->nc"
