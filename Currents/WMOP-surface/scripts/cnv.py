...
ncFile = Dataset("WMOP-surface_Currents_surface_%s.nc" % date,'r')
times = ncFile.variables['ocean_time'][:].data
u = ncFile.variables['u'][:].data
v = ncFile.variables['v'][:].data
lat = ncFile.variables['lat_uv'][:].data
lon = ncFile.variables['lon_uv'][:].data
...
# latUni = np.arange(lat.min(), lat.max(), 0.018)  ## Original lat is not uniform
# latUni = np.linspace(lat.min(), lat.max(), lat.size)
latUni = lat # np.linspace(35, 44.5, lat.size)
lonUni = lon  ##  Original lon is uniform

R = 6378137
x = R * lonUni * np.pi/180.
y = R * np.log( np.tan(np.pi/4 + latUni*np.pi/180/2) )


##  For interpolation
xx = np.linspace(np.min(x), np.max(x), lonUni.size)
yy = np.linspace(np.min(y), np.max(y), latUni.size)
...
for i in np.arange(times.size):
    date = baseTime + timedelta(seconds=times[i])
    uTime = u[i,:,:]
    vTime = v[i,:,:]

    # fu = interpolate.interp2d(x, y, uGlob, kind='linear')
    # fv = interpolate.interp2d(x, y, vGlob, kind='linear')
    fu = interpolate.interp2d(x, y, uTime, kind='linear')
    fv = interpolate.interp2d(x, y, vTime, kind='linear')
    ...
    with open('../png/WMOP-surface_Currents_%s_%s.png' % (date.strftime("%Y%m%d"), date.strftime("%H")), 'wb') as f:
        print("WMOP_Currents_%s_%s.png" % (date.strftime("%Y%m%d"), date.strftime("%H")))
