...
latNC = ncFileWest.variables['lat'][:].data
lonNC = np.concatenate(
    (ncFileWest.variables['lon'][:].data[1:], ncFileEast.variables['lon'][:].data), axis=0)
lonNC[lonNC > 180] -= 360
...
def saveImg(i, j):
    ...
    if(np.min(uNew) == 127 and np.max(uNew) == 127 and np.min(vNew) == 127 and np.max(vNew) == 127):
        ...
        uvColored[:, :, 1] = vNew[:, :]
        uvColored[:, :, 2] = 0
        imageio.imwrite('../tiles/HYCOM_Currents_%s_%02d_%05d-%05d/%d/%d/%d.png' % (date, hr,
