...
# Seaice
ncSeaice = Dataset("Coraltemp_SST_%s.nc" % date, 'r')
seaice = ncSeaice.variables['sea_ice_fraction']
...
def saveImg(i, j):
    try:
        ...
        return
    devNull = os.system('mkdir -p %s/tiles/Coraltemp_Seaice_%s_00/%d/%d' %
                        (path, date, zoom, i))
    ...
    if(np.isnan(np.nanmax(seaiceNew))):
        ...
        # Saving
        imageio.imwrite('%s/tiles/Coraltemp_Seaice_%s_00/%d/%d/%d.png' % (path,date,zoom, i, 2**zoom-j-1), np.flipud(seaiceColored).astype(np.uint8))
