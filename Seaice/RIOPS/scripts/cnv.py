...
# Seaice
ncSeaice = Dataset("CMC_RIOPS_Seaice_%s_%03d.nc" % (date, hr), 'r')
seaice = ncSeaice.variables['iiceconc'][0].data
...
def saveImg(i, j):
    try:
        ...
        return
    devNull = os.system('mkdir -p ../tiles/RIOPS_Seaice_%s_%02d/%d/%d' %
    ...
    if(np.isnan(np.nanmax(seaiceNew))):
        ...
        # Saving
        imageio.imwrite('../tiles/RIOPS_Seaice_%s_%02d/%d/%d/%d.png' % (dateSave, hrSave,zoom, i, 2**zoom-j-1), np.flipud(seaiceColored).astype(np.uint8))
