...
ice = ice[40:681]  ##  Limit lat range
...
iceNew = np.concatenate((iceNew[:,int(len(lon)/2):], iceNew[:,:int(len(lon)/2)]), axis=1)


img = np.empty(iceNew.shape, dtype=np.uint16)
img[:,:] = 2**16*(iceNew-iceMin)/(iceMax-iceMin)

with open('../png/CMC_Seaice_%s_%02d.png' % (datetime.strftime(dateSave, "%Y%m%d"), hrSave), 'wb') as f:
