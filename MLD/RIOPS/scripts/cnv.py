...
# MLD
ncMLD = Dataset("CMC_RIOPS_MLD_%s_%03d.nc" % (date, hr), 'r')
MLD = ncMLD.variables['sokaraml'][0].data
...
MLD[np.isnan(MLD)] = -9999  ##  Since we want to keep sstMin's
...
def saveImg(i, j):
    try:
        ...
        return
    devNull = os.system('mkdir -p ../tiles/RIOPS_MLD_%s_%02d/%d/%d' %
    ...
    if(np.isnan(np.nanmax(MLDnew))):
        ...
        # Saving
        imageio.imwrite('../tiles/RIOPS_MLD_%s_%02d/%d/%d/%d.png' % (dateSave, hrSave,zoom, i, 2**zoom-j-1), np.flipud(MLDcolored).astype(np.uint8))
...
def saveTile():
    ...
    for zoom in np.arange(minZoom, maxZoom+1):
        ...
        iters = np.array(np.meshgrid(np.arange(2**zoom),np.arange(jStart,jEnd))).T.reshape(-1,2)
        #
