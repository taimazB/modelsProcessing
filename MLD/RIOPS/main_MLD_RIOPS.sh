...
export lastAvailDate=`grep MSC_RIOPS_SOKARAML_SFC_PS5km ${HERE}/files | tail -1 | sed 's/.*href=\"\(.*\)T00Z.*\".*/\1/'`
...
function dl {
    ...
    dlLink="${ftpLink}${hr}/${lastAvailDate}T00Z_MSC_RIOPS_SOKARAML_SFC_PS5km_P${hr}.nc"
...
function regrid {
    ...
    cdo -s -O remapbil,${HERE}/scripts/grid.txt ${lastAvailDate}T00Z_MSC_RIOPS_SOKARAML_SFC_PS5km_P${hr}.nc CMC_RIOPS_MLD_${lastAvailDate}_${hr}.nc
    rm ${lastAvailDate}T00Z_MSC_RIOPS_SOKARAML_SFC_PS5km_P${hr}.nc
}
export -f regrid
...
ls /media/taimaz/mapTiles/MLD/RIOPS/tiles | cut -d_ -f3,4 > /media/taimaz/mapTiles/MLD/RIOPS/dateTimes
EOF
    
    log "`date` - MLD RIOPS - rsynched - DONE"
}
...
if [[ ! -z ${lastAvailDate} ]] && [[ ${lastAvailDate} != ${lastDlDate} ]]; then
    ...
    log "`date` - MLD RIOPS - Downloaded"
    echo -e "\tRegridding ..." ; parallel -j 6 "regrid {}" ::: {000..047..3}
    log "`date` - MLD RIOPS - cdo done"
    echo -e "\tProcessing ..."
    for hr in {000..047..3}; do
        py ${hr}
    done
    log "`date` - MLD RIOPS - Converted"
