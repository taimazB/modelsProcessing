...
export tiles=${remote}:/home/taimaz/Projects/server/public/mapTiles/MLD/HYCOM/tiles
...
date -u +%Y%m%dT%H%M%S > /home/taimaz/Projects/server/public/mapTiles/MLD/HYCOM/lastProcessed
EOF

    # rm -r ${ncDir}  ##  Do NOT remove here.  SHF needs it.
    rm -r ${HERE}/tiles
    
    # log "`date` - MLD HYCOM Rsync DONE."
}
...
##################################################
##  nc -> tiles
log "`date` - MLD HYCOM Convertion STARTED"
rm -r ${HERE}/tiles/
mkdir ${HERE}/tiles/
cd ${ncDir}
...
log "`date` - MLD HYCOM Convertion DONE"
